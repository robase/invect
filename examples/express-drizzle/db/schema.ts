...
import type { BatchProvider, CredentialAuthType, CredentialConfig, CredentialType, InvectDefinitionRuntime, JSONValue, TriggerType } from '@invect/core/types';
...
export const flowVersions = sqliteTable('invect_flow_versions', {
  flowId: text('flow_id').notNull().references(() => flows.id, { onDelete: 'cascade' }),
  version: integer('version').notNull(),
  invectDefinition: text('invect_definition', { mode: 'json' }).$type<InvectDefinitionRuntime>().notNull(),
  createdAt: text('created_at').notNull().default(sql`CURRENT_TIMESTAMP`),
...
export const actionTraces = sqliteTable('invect_action_traces', {
  ...
  status: text('status').$type<NodeExecutionStatus>().notNull().default(NodeExecutionStatus.PENDING),
  inputs: text('inputs', { mode: 'json' }).$type<JSONValue>().notNull(),
  outputs: text('outputs', { mode: 'json' }).$type<JSONValue>(),
  error: text('error'),
  startedAt: text('started_at').notNull().default(sql`CURRENT_TIMESTAMP`),
