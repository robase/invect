...
export const SEED_FLOWS = {
  ...
  aiAssistant: {
    ...
    definition: {
      nodes: [
        { id: "n1", type: "core.input", label: "Question", referenceId: "question", params: { variableName: "query", defaultValue: "What is workflow orchestration?" }, position: { x: 80, y: 200 } },
        { id: "n2", type: "core.template_string", label: "Build Prompt", referenceId: "build_prompt", params: { template: "Summarize this topic: {{ question }}" }, position: { x: 340, y: 200 } },
        { id: "n3", type: "AGENT", label: "Research Agent", referenceId: "research_agent", params: { model: "gpt-4o-mini", taskPrompt: "Research: {{ build_prompt }}", maxIterations: 5, enabledTools: [] }, position: { x: 600, y: 200 } },
        { id: "n4", type: "core.output", label: "Answer", referenceId: "answer", params: { outputValue: "{{ research_agent }}" }, position: { x: 860, y: 200 } },
      ],
      edges: [
        { id: "e1", source: "n1", target: "n2" },
        { id: "e2", source: "n2", target: "n3" },
  ...
  simpleTemplate: {
    ...
    definition: {
      nodes: [
        { id: "n1", type: "core.input", label: "Topic", referenceId: "topic", params: { variableName: "subject", defaultValue: "artificial intelligence" }, position: { x: 100, y: 200 } },
        { id: "n2", type: "core.template_string", label: "Format Prompt", referenceId: "format_prompt", params: { template: "Write about {{ topic }} in detail." }, position: { x: 400, y: 200 } },
      ],
      edges: [
        { id: "e1", source: "n1", target: "n2" },
      ],
  ...
  agentEmpty: {
    ...
    definition: {
      nodes: [
        { id: "n1", type: "AGENT", label: "AI Agent", referenceId: "ai_agent", params: {}, position: { x: 300, y: 200 } },
      ],
      edges: [],
    },
  },
  ...
  agentWithTools: {
    ...
    definition: {
      nodes: [
        { id: "n1", type: "core.input", label: "Task", referenceId: "task", params: { variableName: "task", defaultValue: "Analyse the sales data and send a summary" }, position: { x: 80, y: 200 } },
        {
          id: "n2",
          type: "AGENT",
          label: "Data Agent",
        ...
        { id: "n3", type: "core.output", label: "Report", referenceId: "report", params: { outputValue: "{{ data_agent }}" }, position: { x: 640, y: 200 } },
      ],
      edges: [
        { id: "e1", source: "n1", target: "n2" },
        { id: "e2", source: "n2", target: "n3" },
...
export const SCREENS: ScreenDefinition[] = [
  // ── Pages & Navigation ──────────────────────────────────────────────────
  {
    ...
    description: "Dashboard page with sidebar in default collapsed (icon-only) state. Shows flow cards, stats, and navigation icons.",
    tags: ["page", "dashboard", "navigation"],
  },
  {
    id: "02-dashboard-expanded",
    description: "Dashboard page with sidebar expanded showing full nav labels (Home, Executions, Credentials).",
    tags: ["page", "dashboard", "navigation"],
  },
  {
    id: "03-executions-page",
    description: "Executions page showing the flow run history table with status, duration, and timestamp columns.",
  ...
  },
  {
    ...
    description: "Add Credential modal dialog opened from the credentials page, showing credential type selection and form fields.",
    tags: ["modal", "credentials"],
    focusCropSelector: "[role='dialog']",
  },
  {
    id: "06b-credential-edit-modal",
    description: "Credential detail modal with the Edit tab selected, showing editable credential fields and update actions.",
  ...
  {
    id: "07-editor-canvas",
    description: "Flow editor canvas showing the 'Data Pipeline' flow with 3 connected nodes (User Data → Transform → Results) on the React Flow canvas.",
    tags: ["editor", "canvas", "nodes"],
  },
  {
    id: "08-node-selected",
    description: "Flow editor with the 'Transform' (JQ) node clicked/selected, showing selection highlight state.",
    tags: ["editor", "node-selection"],
  },
  {
    id: "09-input-config-panel",
    description: "Node configuration panel for the 'User Data' Input node, showing parameter fields and input/output preview editors.",
    tags: ["editor", "config-panel", "input-node"],
    focusCropSelector: "[role='dialog']",
  },
  {
    id: "10-jq-config-panel",
    description: "Node configuration panel for the 'Transform' JQ node, showing the JQ query code editor and parameter fields.",
    tags: ["editor", "config-panel", "jq-node"],
    focusCropSelector: "[role='dialog']",
  },
  {
    id: "11-agent-config-panel",
    description: "Node configuration panel for the 'Research Agent' AGENT node, showing task prompt, model selector, tool configuration, and iteration settings.",
    tags: ["editor", "config-panel", "agent-node"],
    focusCropSelector: "[role='dialog']",
  },
  {
    id: "12-editor-toolbar",
    description: "Flow editor header and toolbar area showing the flow name, zoom controls, run button, and version info.",
    tags: ["editor", "toolbar"],
  ...
  // ── Chat Assistant ────────────────────────────────────────────────────────
  {
    ...
    description: "Chat assistant panel opened on the flow editor with no LLM credential configured, showing the setup prompt.",
    tags: ["chat", "empty-state", "credentials"],
  },
  {
    id: "14-chat-settings-panel",
    description: "Chat assistant settings panel showing LLM credential selector dropdown and max steps configuration.",
    tags: ["chat", "settings", "credentials"],
  },
  {
    id: "15-chat-ready",
    description: "Chat assistant panel with credential configured, showing the empty conversation ready state with input field.",
  ...
  },
  {
    ...
    description: "Chat assistant panel showing the assistant's reply rendered as markdown, with tool call bubbles showing executed tools.",
    tags: ["chat", "conversation", "assistant-reply", "tool-calls"],
  },
  {
    id: "18-chat-tool-expanded",
    description: "Chat assistant panel with a tool call result expanded, showing the JSON output data inside a collapsible section.",
    tags: ["chat", "conversation", "tool-calls", "expanded"],
  },
  {
    id: "19-chat-multi-turn",
    description: "Chat assistant panel showing a multi-turn conversation with user messages, assistant replies, and multiple tool calls demonstrating the full interaction flow.",
  ...
  // ── Agent Node ───────────────────────────────────────────────────────────
  {
    ...
    description: "Flow editor canvas showing the 'AI Assistant' flow with the AGENT node visible. The AgentToolsBox below the node shows the empty state with the dashed 'Add Tools' call-to-action.",
    tags: ["editor", "canvas", "agent-node", "empty-state"],
  },
  {
    id: "23-agent-node-canvas-with-tools",
    description: "Flow editor canvas showing the 'Agent With Tools' flow. The AGENT node has a populated AgentToolsBox below it displaying tool tiles (Math Evaluate, HTTP Request, JQ Transform) and a Configure button.",
    tags: ["editor", "canvas", "agent-node", "tools", "agent-tools-box"],
  },
  {
    id: "24-agent-config-panel-empty",
    description: "Node configuration panel (dialog) for a bare AGENT node with no params configured. Shows the empty model selector, empty task prompt, and an empty tools section.",
    tags: ["editor", "config-panel", "agent-node", "empty-state"],
    focusCropSelector: "[role='dialog']",
  },
  {
    id: "25-agent-config-panel-seeded",
    description: "Node configuration panel (dialog) for the 'Research Agent' AGENT node from the AI Assistant flow, showing the configured model, task prompt, and max iterations settings.",
    tags: ["editor", "config-panel", "agent-node"],
    focusCropSelector: "[role='dialog']",
  },
  ...
  // ── Tools Configuration ───────────────────────────────────────────────────
  {
    ...
    description: "Flow editor with the left sidebar switched to 'Agent Actions' mode after clicking 'Add Tools' on an agent node with no tools. Shows the tool catalog with search, category filters, and the full list of available actions. No tools are added yet.",
    tags: ["editor", "tools", "agent-actions-sidebar", "empty-state"],
  },
  {
    id: "27-agent-actions-sidebar-seeded",
    description: "Flow editor with the 'Agent Actions' sidebar open on the 'Agent With Tools' flow. Shows the tool catalog with the count badge (3 tools already added), and tool tiles visible in the sidebar list.",
    tags: ["editor", "tools", "agent-actions-sidebar"],
  },
  {
    id: "28-tool-config-panel",
    description: "Flow editor showing the ToolConfigPanel right panel after clicking the 'HTTP Request' tool tile in the AgentToolsBox. The panel shows the tool name, description, category badge, and parameter fields.",
  ...
  // ── Theme ────────────────────────────────────────────────────────────────
  {
    ...
    description: "Dashboard page in dark mode theme, showing flow cards and navigation with dark color scheme.",
