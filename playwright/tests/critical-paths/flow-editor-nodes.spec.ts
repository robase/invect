...
apiBase }) => {
  ...
  // ── Test 2 — Critical ─────────────────────────────────────────────────────
  test("canvas nodes can be selected by clicking", async ({ 
    page,
    navigateToFlow,
apiBase }) => {
    // 1. Navigate to the JQ Data Transform flow editor
    ...
    expect(
      initialCount,
      "Canvas must have at least one node"
  ...
  test("save flow persists to the backend", async ({ page, request, apiBase }) => {
    ...
    try {
      ...
      expect(
        getResp.ok(),
        `GET /flows/${flow.id} should return 2xx after save`
      ).toBeTruthy();
...
apiBase }) => {
    ...
    await expect(async () => {
      ...
      expect(
        count,
        "JQ Data Transform should have exactly 3 nodes on the canvas"
      ).toBe(3);
