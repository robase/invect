...
const TEST_FLOW_DEFINITIONS: Record<string, TestFlowDefinition> = {
  ...
  "User Age Check (Adult)": {
    nodes: [
      ...
      },
      {
        ...
        params: {
          template: "Hi {{ age_check.user_info.name }}! Some features are restricted for users under 18.",
        },
    ...
    edges: [
      ...
      { id: "edge-ifelse-true", source: "if-adult", target: "template-adult", sourceHandle: "true_output" },
      { id: "edge-ifelse-false", source: "if-adult", target: "template-minor", sourceHandle: "false_output" },
    ],
  },
  ...
  requiresHotfix: (.incident.priority == "P1" or .incident.impactedCustomers > 100)
    ...
    edges: [
      ...
      { id: "ops-exec-true", source: "if-executive-update", target: "template-executive-brief", sourceHandle: "true_output" },
      { id: "ops-exec-false", source: "if-executive-update", target: "if-hotfix", sourceHandle: "false_output" },
      { id: "ops-hotfix-true", source: "if-hotfix", target: "template-hotfix-plan", sourceHandle: "true_output" },
      { id: "ops-hotfix-false", source: "if-hotfix", target: "template-monitoring-plan", sourceHandle: "false_output" },
      { id: "ops-brief-output", source: "template-executive-brief", target: "output-dispatch-summary" },
      { id: "ops-hotfix-output", source: "template-hotfix-plan", target: "output-dispatch-summary" },
      { id: "ops-monitor-output", source: "template-monitoring-plan", target: "output-dispatch-summary" },
    ],
  },
...
async function deleteRecordsByName(
  resourcePath: "/credentials" | "/flows",
  name: string,
) {
...
async function navigateToFlow(page: Page, flowName: string) {
  ...
  await expect(page.getByText("Loading flows")).not.toBeVisible({ timeout: 15_000 }).catch(() => {});
...
async function runNodeByName(page: Page, nodeName: string) {
  ...
  if (!isVisibleOnCanvas) {
    const node = activeTestFlow?.definition.nodes.find(
      (candidate) => candidate.label === nodeName,
    );
...
export function assertNoObjectObject(text: string, context: string) {
  expect(text, `${context} should not contain [object Object]`).not.toContain(
    "[object Object]"
  );
}
