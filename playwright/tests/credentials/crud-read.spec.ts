...
test.describe("Credential CRUD — Read & Detail View", () => {
  test("view credential detail panel shows overview metadata", async ({
    page,
  }) => {
    ...
    await expect(
      dialog.getByRole("heading", { name: "Anthropic API Key" })
    ).toBeVisible();
  ...
  test("search filters credentials by name", async ({ page }) => {
    ...
    await expect(
      page.getByText("No credentials match your search.")
    ).toBeVisible();
  });
  ...
  test("detail dialog has Overview, Edit, and Webhook tabs", async ({
    page,
  }) => {
