...
test.describe("Webhook Ingestion — Trigger a Flow via Credential Webhook", () => {
  test("POST to credential webhook URL triggers the associated flow", async ({
    ...
    const infoResp = await request.get(
      `${apiBase}/credentials/${credId}/webhook-info`
    );
    ...
    const webhookResp = await request.post(
      `${apiBase}/webhooks/credentials/${webhookPath}`,
      {
  ...
  test("webhook to unknown path returns 404 or appropriate error", async ({
    request,
    apiBase,
  }) => {
    // 1. Send a POST request to a nonexistent webhook path
    const resp = await request.post(
      `${apiBase}/webhooks/credentials/nonexistent-path-abc123`,
      {
        data: { test: true },
        headers: { "Content-Type": "application/json" },
      }
