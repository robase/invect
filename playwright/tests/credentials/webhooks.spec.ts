...
test.describe('Credential Webhooks — Enable & Display', () => {
  test('enable webhook on a credential and view URL and secret', async ({
    ...
    await expect(dialog.getByText(/\/webhooks\/credentials\//)).toBeVisible({ timeout: 10_000 });

    // Secret field is displayed
    await expect(dialog.getByText('Secret').first()).toBeVisible();

    // 4. Descriptive text
    await expect(dialog.getByText(/External services send events to this URL/)).toBeVisible();
  });
  ...
  test('existing webhook-enabled credential shows URL on webhook tab', async ({
    ...
    await request.post(`${apiBase}/credentials/${linearCred!.id}/webhook/enable`);

    // 1. Navigate and click on 'Linear OAuth2'
    await goToCredentials(page);
    const row = page.getByRole('button').filter({ hasText: 'Linear OAuth2' });
    await row.click();
    ...
    await expect(dialog.getByRole('button', { name: 'Enable Webhook' })).not.toBeVisible();
