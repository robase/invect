...
test.describe("Credential Edge Cases & Error Handling", () => {
  test("empty state shows create prompt when no credentials exist", async ({
    ...
    const existingCreds: Array<{ id: string; name: string }> =
      await listResp.json();
    ...
    try {
      ...
      await expect(
        page.getByText(/Add API keys, connect OAuth providers/)
      ).toBeVisible();
      ...
      await request.post(`${apiBase}/credentials`, {
        data: {
          ...
          description:
            "Linear OAuth2 credential for issue tracking and project management",
        },
      });
    }
  });

  test("duplicate credential names are allowed", async ({
    page,
    request,
    apiBase,
  }) => {
  ...
  test("credential config secrets are masked in password fields", async ({
    page,
  }) => {
