...
The configuration object is passed to `createInvectRouter()` (Express), `InvectModule.forRoot()` (NestJS), or `createInvectHandler()` (Next.js). It is also read directly by `<Invect config={...} />` on the frontend for fields like `apiPath`, `frontendPath`, `theme`, and `plugins`. Use `defineConfig()` from `@invect/core` for type inference.
...
export const invectConfig = defineConfig({
  database: { ... },
  apiPath: '/api/invect',   // backend mount path
  frontendPath: '/invect',  // where <Invect> is mounted in the frontend router
...
plugins: [
  ...
  rbac(),
  webhooks({ webhookBaseUrl: 'https://api.myapp.com/invect' }),
];
```
...
The URL path where the Invect API is mounted (e.g. `'/api/invect'` or `'http://localhost:3000/invect'`). Read by `<Invect>` on the frontend to know where to send API requests.

## `frontendPath` (optional)

The URL path where `<Invect>` is mounted in the frontend router (e.g. `'/invect'`). Used for internal navigation and the OAuth callback route (`<frontendPath>/oauth/callback`).
