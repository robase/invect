...
import { actionToNodeDefinition } from '../action-registry';
...
async function loadModelOptions(
  ...
  // Lazily import adapters to avoid circular deps
  const { OpenAIAdapter } = await import('src/services/ai/openai-adapter');
  const { AnthropicAdapter } = await import('src/services/ai/anthropic-adapter');

  let models: Model[];
  let defaultModelId: string;

  if (provider === BatchProvider.OPENAI) {
    const adapter = new OpenAIAdapter(ctx.logger, apiKey);
    const raw = await adapter.listModels();
    models = raw;
    defaultModelId = adapter.defaultModel;
  } else if (provider === BatchProvider.ANTHROPIC) {
    const adapter = new AnthropicAdapter(ctx.logger, apiKey);
    const raw = await adapter.listModels();
    models = raw;
    defaultModelId = adapter.defaultModel;
  } else {
    return { options: [], placeholder: 'Unsupported provider' };
  }
