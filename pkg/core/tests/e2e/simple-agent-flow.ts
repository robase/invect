 ...
 */
function buildSimpleAgentFlowDefinition(
  credentialId: string,
  isOpenAI: boolean
): InvectDefinition {
...
Use the math_eval tool if you need to perform any calculations. Be precise and show your work.`,
          systemPrompt: "You are a helpful math assistant. Use the math_eval tool to perform accurate calculations. Always show your reasoning.",
...
export const simpleAgentFlowExample: FlowExample = {
      ...
      if (agentOutput) {
        ...
        if (agentOutput.toolResults?.length > 0) {
          for (const toolResult of agentOutput.toolResults) {
            console.log(`     - ${toolResult.toolName}: ${toolResult.success ? 'success' : 'failed'}`);
  ...
  expected(result) {
    ...
    const responseContainsResult = finalResponse.includes('127.05') || 
                                   finalResponse.includes('127');
    assert.ok(responseContainsResult, "Agent response should contain the correct calculation result");
